...
# Lance sans pause pour voir le résultat final directement
test_with_features(super_simple_separable_through_origin, order=2, draw=True, pause=False)
...
raw_data, labels = super_simple_separable()
phi = make_polynomial_feature_fun(3)
data_transformed  = phi(raw_data)

def hook_jupyter(params):
    th, th0 = params
    clear_output(wait=True)
    ax = plot_data(raw_data, labels)
    plot_nonlin_sep(
        lambda x1, x2: int(positive(phi(cv([x1, x2])), th, th0)),
        ax=ax
    )
    plt.show()
    print(f"th={th.T}  th0={th0}")

theta, theta0 = perceptron(data_transformed, labels,
                            params={'T': 100},
                            hook=hook_jupyter)

print(f"Score final : {int(score(data_transformed, labels, theta, theta0))}")

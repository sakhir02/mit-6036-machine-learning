...
np.array([[ 0.87763576, -0.83262048,-2.04501529],
...
#definition de relu
def Relu(z):
    return np.maximum(0, z)   # ✅ propre et vectorisé
a=Relu(z)
w2=np.array([[1,-1],[1,-1],[1,-1],[1,-1]])
w02=np.array([[0,2]]).T
z2=np.dot(w2.T,a)+w02
z2
def softmax(z):
    somme = sum(np.exp(z))
    return np.exp(z)/somme
softmax(z2)
...
class Linear(Module):
    ...
    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        ...
        self.dLdW0 = np.sum(dLdZ, axis=1, keepdims=True)  # ✅ shape (n,1)  # Your code
...
class SoftMax(Module):  # Output activation
    def forward(self, Z):
        self.Z=Z
        e = np.exp(self.Z)
        return e / np.sum(e, axis=0, keepdims=True)  # Your code: (?, b)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):
        ...
        for it in range(iters):
            # ✅ Tire un seul exemple aléatoire à chaque itération
            idx = np.random.randint(N)
            x_batch = X[:, idx:idx+1]   # (D, 1)
            y_batch = Y[:, idx:idx+1]   # (K, 1)
            
            Ypred = self.forward(x_batch)
            cur_loss = self.loss.forward(Ypred, y_batch)
